...
export class Multiple extends Component  {
  ...
  render = props => {
    props = mergeProps(
     ...
     const allSelectedLeafKeys = (() => {
       let leafKeys: string[] = []
       this.data.value.forEach(v => {
     ...
     const renderItem = (option: TreeSelectOption) => {
       const isExpand = this.data.expandKeys.includes(option[valueName])
         ...
         const column = (
           ...
           >
             {renderItems(
               i === 0
                 ? props.options
                 : optionsMap.get(this.data.expandKeys[i - 1])?.[childrenName],
               i
             )}
           </div>
         )
